...
if opcion == 'Analisis Exploratorio':
    ...
    if 'df' in st.session_state:
        ...
        if 'df_normalizado' in st.session_state:
            ...
            plt.hlines(y=0.9,xmin=1,xmax=5,colors='r',linestyles='--')
        ...
        if 'df_normalizado' in st.session_state:
            ...
            num_clusters = st.slider('Seleccione el número de clusters', min_value=2, max_value=10, value=3)
    
            if st.button('Aplicar K-Means'):
                with st.spinner('Aplicando K-Means...'):
                    kmeans = KMeans(n_clusters=num_clusters)
                    clusters = kmeans.fit_predict(df_normalizado)
                    df_kmeans = df_normalizado.copy()
                    df_kmeans['Cluster'] = clusters
                    st.write('Datos después del clustering:')
                    st.write(df_kmeans.head())
            
                    # Visualización
                    st.subheader('Visualización de Clusters')
                    fig, ax = plt.subplots()
                    sns.scatterplot(x=df_kmeans[eje_x], y=df_kmeans[eje_y], hue=df_kmeans['Cluster'], palette='tab10', ax=ax)
                    st.pyplot(fig)
    else:
        st.warning('No se ha cargado ningun archivo')